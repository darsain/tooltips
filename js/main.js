...
// Hoverable
var tips = window.tips = new Tooltips($example[0], {
	tooltip: {
		auto: 1,
		typeClass: 'light',
		effectClass: 'slide'
...
// Focusable
window.hints = new Tooltips($example[0], {
	tooltip: {
		auto: 1,
		typeClass: 'light',
		effectClass: 'slide'
...
// Dynamic elements
var $container = $example.find('.container');
$example.on('click', '[data-action]', function () {
	var action = $(this).data('action');
	switch (action) {
		case 'add':
			var $added = $('<li class="target" data-tip="A new tip appeared!">new</li>');
			$container.append($added);
			tips.add($added[0]);
			break;
		case 'remove':
			var $removed = $container.children().eq(-1).remove();
			if ($removed.length) {
				tips.remove($removed[0]);
			}
			break;
	}
});
